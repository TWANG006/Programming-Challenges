...
way that text editors allow us to modify documents. Images are represented as an M¡ÁN
...
and a row number between 1. . . N, where 1 ¡Ü M, N ¡Ü 250. The origin sits in the
upper-left corner of the table. Colors are specified by capital letters.
The editor accepts the following commands:

I M N			Create a new M ¡ÁN image with all pixels initially colored white (O).
C				Clear the table by setting all pixels white (O). The size remains unchanged.
L X Y C			Colors the pixel (X, Y ) in color (C).
V X Y1 Y2 C		Draw a vertical segment of color (C) in column X, between the rows 
				Y1 and Y2 inclusive.
H X1 X2 Y C		Draw a horizontal segment of color (C) in the row Y, between the
				columns X1 and X2 inclusive.
K X1 Y1 X2 Y2 C Draw a filled rectangle of color C, where (X1, Y 1) is the upper-left 
				and (X2, Y 2) the lower right corner.
F X Y C			Fill the region R with the color C, where R is defined as follows. 
				...
				pixel (X, Y ) and shares a common side with any pixel in R also belongs 
				to this region.
...
output. Ignore the entire line of any command defined by a character other than I, C, L,
